<template>
	...
	<div v-resize="onResize">
		<!-- Menu de gauche avec la liste des pages -->
		<v-navigation-drawer
			v-model="drawer"
			absolute
			temporary
		>
			<v-list nav class="overflow-y-auto" :max-height=height>
...
<!-- ***************************** SHOP ******************* -->
		<div v-if="displaySet">
			<v-list class="overflow-y-auto" :max-height=height >
				<v-list-item>
			...
			<v-dialog
				...
				width="500"
			>
				<!-- Bouton pour afficher le checkout -->
		...
		pay(){
			...
			fetch(this.config.ip + "/checkout", request)
				...
				.then(data => {
					...
					this.dialog = false;
					window.location.href = this.config.moneyAppIp + "/?checkout_id=" + checkoutId + "?user_id=" + this.searchUser ;
				})
